...
class _HomeScreenState extends State<HomeScreen>
      ...
      body: AnnotatedRegion<SystemUiOverlayStyle>(
        ...
        child: SafeArea(
          child: CustomScrollView(
            ...
            slivers: [
              // Header fixe — reste affiché pendant le scroll
              ...
              SliverPadding(
                ...
                sliver: _settings.userMode == UserMode.user
                    ...
                    : _buildAdaptiveFeatureGrid(context),
              ),
              const SliverToBoxAdapter(child: SizedBox(height: 24)),
              if (_settings.userMode != UserMode.user)
                SliverPadding(
                  padding: const EdgeInsets.fromLTRB(24, 0, 24, 24),
                  sliver: SliverToBoxAdapter(child: _buildStatsCard(context)),
                ),
            ],
          ),
        ),
      ...
      floatingActionButton: _settings.userMode == UserMode.user
          ? FloatingActionButton.extended(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  _createRoute(const ViewConfigsScreen()),
                );
              },
  ...
  String _extractMovement(String payload) {
    ...
    if (trimmed.startsWith('{')) {
      try {
        ...
        if (decoded is Map) {
          final movement =
              decoded['movement'] ?? decoded['mouvement'] ?? decoded['move'];
  ...
  Widget _buildStatsCard(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final deviceCount = _mqtt.deviceStates.length;
    final lastMsg = _mqtt.recentMessages
        .where((m) =>
            m.isIncoming &&
            (m.topic == 'home/wristband/move' ||
                m.topic == 'home/wrisband/move'))
        .firstOrNull;
          ...
          children: [
            Row(
              children: [
                stat(
                  ...
                  value: '${_settings.wristbandConfigsCount}',
                  label: 'Configs',
                ),
                Container(
                  width: 1,
            ...
            if (lastMsg != null) ...[
              ...
              Align(
                ...
                child: Text(
                  _extractMovement(lastMsg.message),
                  style: TextStyle(
                    color: isDark ? Colors.white : Colors.black87,
                    fontSize: 15,
                    fontWeight: FontWeight.w600,
  ...
  Widget _buildMonitoringSliverGrid(BuildContext context) {
    final devices = _mqtt.isConnected
        ? _mqtt.deviceStates.values.toList()
        : <DeviceState>[];
    final isDark = Theme.of(context).brightness == Brightness.dark;
    ...
    return SliverGrid(
      ...
      delegate: SliverChildBuilderDelegate((context, index) {
        ...
        final timeText =
            '${ds.lastUpdated.hour.toString().padLeft(2, '0')}:${ds.lastUpdated.minute.toString().padLeft(2, '0')}';
        Color iconBg;
        try {
        ...
        final staggerMs = (index * 70).clamp(0, 350);

        return TweenAnimationBuilder<double>(
          key: ValueKey('device-bounce-${ds.topic}'),
          tween: Tween(begin: 0.0, end: 1.0),
          duration: Duration(milliseconds: 500 + staggerMs),
          curve: Curves.easeOutBack,
          builder: (context, value, child) {
            return Transform.scale(scale: value, child: child);
          },
          child: IOSCard(
            ...
            child: GestureDetector(
              onTap: _isLamp(ds)
                  ? () => _showLightControlDialog(context, ds)
                  : null,
              child: Column(
                ...
                children: [
                  ...
                  Center(
                    child: Container(
                      ...
                      child: Icon(displayIcon, size: 32, color: iconColor),
                    ),
                  ),
                  const SizedBox(height: 8),
                  // Nom et type
  ...
  void _showLightControlDialog(BuildContext context, DeviceState device) {
    ...
    Color currentColor = Color(
      int.parse(device.displayColor.replaceFirst('#', '0xFF')),
    );
  ...
  Widget _buildAdaptiveFeatureGrid(BuildContext context) {
    final cards = _buildFeatureCards(context);
    const spacing = 16.0;
    ...
    for (var i = 0; i < cards.length; i += 2) {
      ...
      if (isLast) {
        // Tuile seule → pleine largeur, hauteur ≈ celle d'une tuile jumelée
        rows.add(AspectRatio(aspectRatio: 2.1, child: cards[i]));
      } else {
        rows.add(
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(child: AspectRatio(aspectRatio: 1.0, child: cards[i])),
              const SizedBox(width: spacing),
              Expanded(
                child: AspectRatio(aspectRatio: 1.0, child: cards[i + 1]),
              ),
            ],
          ),
        );
    ...
    return SliverToBoxAdapter(child: Column(children: rows));
  }
  ...
  List<Widget> _buildFeatureCards(BuildContext context) {
    ...
    cards.add(
      _buildFeatureCard(
        ...
        onTap: () {
          HapticFeedback.lightImpact();
          Navigator.push(context, _createRoute(const MonitoringScreen()));
        },
      ),
    );
    ...
    if (userMode == UserMode.developer) {
      cards.add(
        _buildFeatureCard(
          ...
          onTap: () {
            HapticFeedback.lightImpact();
            Navigator.push(context, _createRoute(const MqttControlPage()));
          },
        ),
      );
    }
...
class _HomeHeaderDelegate extends SliverPersistentHeaderDelegate {
  ...
  static const double _height = 152.0;
  ...
  @override
  bool shouldRebuild(_HomeHeaderDelegate old) => true;
              ...
              AnimatedBuilder(
                ...
                builder: (context, _) {
                  return Row(
                    children: [
                      IOSStatusBadge(
                        text: mqtt.isConnected
                            ? settings.text('connected_label')
                            : settings.text('disconnected_label'),
                        color: mqtt.isConnected ? AppTheme.successGreen : AppTheme.errorRed,
                      ...
                      ValueListenableBuilder<BatteryStatus?>(
                        valueListenable: mqtt.batteryStatusNotifier,
                        builder: (context, batteryStatus, _) {
                          ...
                          return IOSStatusBadge(
                            ...
                            color: batteryStatus.batteryLevel > 20
