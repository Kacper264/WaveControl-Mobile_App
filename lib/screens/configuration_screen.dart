...
class _ConfigurationScreenState extends State<ConfigurationScreen> {
  ...
  String?
  _lastProcessedMessageId; // Pour éviter de traiter le même message deux fois
  bool _hasShownAutoWizard =
      false; // Pour éviter d'ouvrir le wizard plusieurs fois automatiquement
  ...
  void _onMqttChanged() {
    ...
    if (messages.isNotEmpty) {
      ...
      final messageId =
          '${lastMessage.topic}_${lastMessage.message}_${lastMessage.timestamp}';
    ...
    try {
      ...
      if (topic == 'home/IR/feedback') {
        ...
        if (status == 'saved_list') {
          final telecommandes =
              jsonData['telecommandes'] as Map<String, dynamic>;
      ...
      switch (topic) {
        ...
        case 'home/wristband/config_status':
          ...
          setState(() {
            if (status.status != 'success') {
              _statusMessage =
                  '${_settings.text('send_error')}: ${status.message}';
              _statusType = 'error';
            }
            _isLoading = false;
          });
        ...
        case 'home/wristband/execution_status':
          ...
          print(
            'Execution status: ${execution.status} for ${execution.movement}',
          );
          break;
      }
    } catch (e) {
  ...
  void _saveConfigsLocally() {
    _settings.updateWristbandConfigsCount(_wristbandConfigs.length);
  }
  ...
  void _sendConfiguration() {
    if (!_mqtt.isConnected) {
      setState(() {
        _statusMessage =
            '${_settings.text('not_connected_mqtt')} - ${_settings.text('check_connection')}';
        _statusType = 'error';
      });
    ...
    setState(() => _isLoading = true);
    final configJson =
        '[${_wristbandConfigs.map((c) => '{mouvement:${c.mouvement},entity_id:${c.entityId},action_type:${c.actionType}}').join(',')}]';
  ...
  List<WristbandConfig> _parseConfigList(String configStr) {
    ...
    return (jsonList as List)
        .map((item) => WristbandConfig.fromJson(item))
        .toList();
  }
  ...
  List<WristbandConfig> _dedupeConfigs(List<WristbandConfig> configs) {
    ...
    for (final config in configs) {
      ...
      final actionsForDevice = usedActionsPerDevice.putIfAbsent(
        entityKey,
        () => <String>{},
      );

      if (usedMovements.contains(movementKey))
        continue; // ignore doublon de mouvement
      if (actionsForDevice.contains(actionKey))
        continue; // ignore doublon action par périphérique
  ...
  void _addConfig(String mouvement, String entityId, String actionType) {
    ...
    final duplicateActionIndex = _wristbandConfigs.indexWhere(
      (config) =>
          _normalizeEntityId(config.entityId) == entityKey &&
          _normalizeAction(config.actionType) == normalizedAction,
    );
  ...
  void _removeConfig(String mouvement) {
    setState(() {
      _wristbandConfigs.removeWhere(
        (config) =>
            _normalizeMovement(config.mouvement) ==
            _normalizeMovement(mouvement),
      );
    });
    // Envoyer automatiquement la configuration mise à jour
    _sendConfiguration();
  ...
  IconData _getIconForMovement(String movement) {
    return getMovementIcon(movement);
  }
  ...
  bool _deviceHasFreeAction(WristbandDevice device) {
    ...
    final usedActions = _wristbandConfigs
        .where(
          (c) =>
              _normalizeEntityId(c.entityId) ==
              _normalizeEntityId(device.entityId),
        )
  ...
  List<String> _getRemainingIrActions(String deviceName) {
    ...
    final usedActions = _wristbandConfigs
        .where(
          (c) =>
              _normalizeEntityId(c.entityId) ==
              _normalizeEntityId('ir:$deviceName'),
        )
        .map((c) => _normalizeAction(c.actionType))
        .toSet();

    return touches
        .where((t) => !usedActions.contains(_normalizeAction(t)))
        .toList();
  }
  ...
  void _showAvailableDevices() {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            _AvailableDevicesScreen(devices: _wristbandPossibility!.devices),
      ),
    );
  }
      ...
      body: _isLoading && _wristbandPossibility == null
          ...
          : ListView(
              ...
              padding: EdgeInsets.fromLTRB(
                ...
                20,
                20,
                _hasLoadedConfigs && _wristbandPossibility != null ? 200 : 20,
              ),
              children: [
                ...
                if (_wristbandConfigs.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 16),
                  ..._wristbandConfigs.map(
                    (config) => TweenAnimationBuilder<double>(
                      tween: Tween(begin: 0.0, end: 1.0),
                      duration: const Duration(milliseconds: 400),
                      curve: Curves.easeOut,
                      builder: (context, value, child) {
...
class _AvailableDevicesScreen extends StatefulWidget {
  ...
  const _AvailableDevicesScreen({required this.devices});

  @override
  State<_AvailableDevicesScreen> createState() =>
      ...
      body: ListView(
        ...
        children: [
          ...
          if (_mqtt.isConnected && _mqtt.deviceStates.isNotEmpty) ...[
            ...
            const SizedBox(height: 16),
            ..._mqtt.deviceStates.values.toList().map(
              (device) => Container(
                margin: const EdgeInsets.only(bottom: 12),
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(
                        (device.friendlyName?.toLowerCase().contains('lum') ??
                                    false) ||
                                (device.topic.toLowerCase().contains('light') ||
                                    device.topic.toLowerCase().contains('lum'))
                    ...
                    IOSStatusBadge(
                      ...
                      color: device.state.toUpperCase() == 'ON'
                          ? AppTheme.successGreen
                          : Colors.grey,
                      icon: device.state.toUpperCase() == 'ON'
                          ? Icons.check_circle_rounded
                  ...
                  return await showDialog(
                        ...
                        builder: (context) => AlertDialog(
                          backgroundColor: isDark
                              ...
                              : Colors.white,
                          title: Text(
                            '${_settings.text('delete')} "${device['name']}" ?',
                          ),
                          content: Text(
                            _settings.text('delete_remote_message'),
                          ),
                          actions: [
                            TextButton(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if ((device['touches'] as List?)?.isNotEmpty ??
                            false) ...[
...
class _ConfigWizardState extends State<_ConfigWizard> {
  ...
  List<String> _getRemainingActionsForDevice() {
    ...
    if (isIr) {
      ...
      try {
        device = widget.wristbandPossibility.devices.firstWhere(
          (d) => d.entityId == _selectedDeviceId,
        );
      } catch (_) {}
      if (device != null) {
          ...
          Column(
            children: [
              Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(_stepIcon(), color: AppTheme.primaryPurple, size: 28),
                    ...
                    .map(
                      (movement) => Padding(
                        ...
                        child: IOSButton(
                          ...
                          iconWidget: buildMovementPictogram(
                            movement,
                            color: Colors.white,
                            size: isTextMovementPictogram(movement) ? 20 : 26,
                          ),
                          iconSize: 26,
          ...
          Row(
            children: [
              ...
              Expanded(
                child: IOSButton(
                  text: _step == 2
                      ? _settings.text('validate')
                      : _settings.text('next'),
                  color:
                      ((_step == 0 && _selectedMovement != null) ||
